...
export function SettingsPreferencesForm({
  ...
  const cardClass =
    "rounded-[2rem] border border-slate-200/80 bg-white/90 p-6 shadow-lg shadow-slate-200/60 backdrop-blur sm:p-8";
  const sectionTitleClass =
    "text-2xl font-semibold tracking-tight text-slate-950";
  const helperTextClass = "text-sm leading-6 text-slate-600";
  const selectClass =
    "mt-2 block w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-slate-900 shadow-sm outline-none transition focus:border-cyan-500 focus:ring-4 focus:ring-cyan-100 disabled:cursor-not-allowed disabled:bg-slate-100";
  const checkboxClass =
    "h-4 w-4 rounded border-slate-300 text-cyan-700 focus:ring-cyan-500";
  const secondaryButtonClass =
    "inline-flex items-center justify-center rounded-full border border-slate-300 bg-white px-5 py-3 text-sm font-semibold text-slate-700 transition hover:border-slate-400 hover:bg-slate-50 disabled:cursor-not-allowed disabled:border-slate-200 disabled:bg-slate-100 disabled:text-slate-400";
  const primaryButtonClass =
    "inline-flex items-center justify-center rounded-full bg-slate-950 px-5 py-3 text-sm font-semibold text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:bg-slate-400";
  const infoRowClass =
    "rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700";
  const [homeCityId, setHomeCityId] = useState(
    initialProfile.home_city_id ?? "",
  );
  const [notificationsEnabled, setNotificationsEnabled] = useState(
    initialProfile.notifications_enabled,
  );
  ...
  const [exportRequest, setExportRequest] =
    useState<ExportRequestRecord | null>(null);
  const [deletionRequest, setDeletionRequest] =
    useState<DeletionRequestRecord | null>(null);
  const [deletionMode, setDeletionMode] =
    useState<DeletionRequestMode>("anonymize");
  ...
  const cityOptions = useMemo(
    () =>
      supportedCityIds.map((cityId) => ({
        cityId,
        label: formatCityLabel(null, cityId),
      })),
    [supportedCityIds],
  );
    ...
    void (async () => {
      try {
        ...
        const matchingSubscription = endpoint
          ? (backendSubscriptions.items.find(
  ...
  const pushCapabilityState = mapPushCapabilityState(
    isPushSupported,
    pushPermission,
  );
  ...
  async function syncBackendSubscription(endpoint: string): Promise<void> {
    ...
    const matchingSubscription =
      subscriptions.items.find((item) => item.endpoint === endpoint) ?? null;
  ...
  async function upsertSubscriptionForDevice(
  ...
  async function applyUpdate(
    nextPausedUntil: string | null = notificationsPausedUntil,
  ) {
    if (!homeCityId) {
  ...
  async function onUnsubscribePush() {
    ...
    try {
      ...
      if (pushSubscriptionId) {
        ...
        const subscriptions = await listPushSubscriptions(authToken);
        const matchingSubscription =
          subscriptions.items.find((item) => item.endpoint === endpoint) ??
          null;
    ...
    try {
      ...
      if (error instanceof GovernanceApiError && error.status === 422) {
        setGovernanceErrorMessage(
          "Unable to submit export request. Check request details and try again.",
        );
      } else {
        setGovernanceErrorMessage(
          "Unable to submit export request. Try again.",
        );
      }
    } finally {
  ...
  async function onRefreshExportStatus() {
    ...
    try {
      ...
      setGovernanceErrorMessage(
        "Unable to refresh export request status. Try again.",
      );
    } finally {
      setIsGovernanceSubmitting(false);
    }
  ...
  async function onRequestDeletion() {
    ...
    try {
      ...
      if (error instanceof GovernanceApiError && error.status === 422) {
        setGovernanceErrorMessage(
          "Unable to submit deletion request. Check request details and try again.",
        );
      } else {
        setGovernanceErrorMessage(
          "Unable to submit deletion request. Try again.",
        );
      }
    } finally {
      setIsGovernanceSubmitting(false);
  ...
  async function onRefreshDeletionStatus() {
    ...
    try {
      ...
      setGovernanceErrorMessage(
        "Unable to refresh deletion request status. Try again.",
      );
    } finally {
      setIsGovernanceSubmitting(false);
    }
    ...
    <main className="mx-auto flex w-full max-w-6xl flex-col gap-6 lg:gap-8">
      <section className="rounded-[2rem] border border-slate-200/80 bg-slate-950 px-6 py-8 text-white shadow-2xl shadow-slate-400/20 sm:px-8">
        ...
        <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-300 sm:text-base">
          Manage your home city, alerts, push subscriptions, and data governance
      ...
      <section className={cardClass}>
        <div className="space-y-2">
          ...
          <p className={helperTextClass}>
            Choose your primary city and how CouncilSense should contact you.
          </p>
        </div>
        ...
        <form onSubmit={onSubmit} className="mt-8 space-y-6">
          ...
          <p className={infoRowClass}>
            ...
            {notificationsPausedUntil ?? "Not paused"}
          </p>

          <div className="flex flex-wrap gap-3">
            <button
              type="button"
              onClick={onPauseFor24Hours}
              disabled={isSubmitting}
      ...
      <section aria-label="Push notifications" className={cardClass}>
        <div className="space-y-2">
          ...
          <p className={helperTextClass}>
            Manage browser-based alerts for the device you are using right now.
          </p>
        </div>
        {pushCapabilityState === "unsupported" ? (
          <p className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4 text-sm text-slate-600">
            Push is not supported in this browser. You can still manage
          ...
          <div className="mt-6 space-y-4">
            ...
            <p className={infoRowClass}>
              Device subscription:{" "}
              {isPushSubscribed ? "Enabled" : "Not enabled"}
            </p>
            <p className={infoRowClass}>
              Server subscription state: {pushServerStatus ?? "none"}
            </p>
            {pushCapabilityState === "permission_denied" ? (
              <p className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-4 text-sm text-amber-800">
                Push is blocked by browser permission. Enable notifications in
                browser settings, then retry.
              </p>
            ) : null}
            {pushRecoveryAction === "resubscribe" ? (
              <p className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-4 text-sm text-amber-800">
                This subscription is no longer deliverable. Recover it to
                continue push alerts.
              </p>
            ) : null}
            {pushRecoveryAction === "reactivate" ? (
              <p className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-4 text-sm text-amber-800">
                This subscription is currently suppressed. Reactivate push to
                resume deliveries.
              </p>
            ) : null}
            <div className="flex flex-wrap gap-3">
              ...
              {pushRecoveryAction ? (
                <button
                  ...
                  disabled={
                    isPushSubmitting || pushCapabilityState !== "subscribable"
                  }
                  className={secondaryButtonClass}
                >
                  {pushRecoveryAction === "reactivate"
                    ? "Reactivate push"
                    : "Recover push subscription"}
                </button>
              ) : null}
            </div>
          </div>
      ...
      <section aria-label="Data governance" className={cardClass}>
        <div className="space-y-2">
          ...
          <p className={helperTextClass}>
            Request exports and account deletion actions with clear status
            tracking.
          </p>
        </div>
        ...
        <div className="mt-8 grid gap-6 xl:grid-cols-2">
          <section
            aria-label="Data export request"
          ...
          >
            ...
            <p className="mt-2 text-sm leading-6 text-slate-600">
              Request an export of your profile, preferences, and notification
              history.
            </p>
            ...
            <div className="mt-4 space-y-3">
              <p className={infoRowClass}>
                ...
                {exportRequest?.status ?? "not requested"}
              </p>
              {exportRequest?.completed_at ? (
                <p className={infoRowClass}>
                  Export completed at: {exportRequest.completed_at}
                </p>
              ) : null}
              {exportRequest?.artifact_uri ? (
                <p className={infoRowClass}>
                  Export artifact: {exportRequest.artifact_uri}
                </p>
              ) : null}
              {exportRequest?.error_code ? (
                <p className={infoRowClass}>
                  Export error code: {exportRequest.error_code}
                </p>
              ) : null}
            </div>
          </section>
          ...
          >
            ...
            <p className="mt-2 text-sm leading-6 text-slate-600">
              Submit a deletion or anonymization request for your account data.
            </p>

            <div className="mt-4">
              <label
                htmlFor="deletion-mode"
                className="block text-sm font-medium text-slate-700"
              >
                Request mode
              </label>
              <select
                ...
                value={deletionMode}
                onChange={(event) =>
                  setDeletionMode(event.target.value as DeletionRequestMode)
                }
                disabled={isGovernanceSubmitting}
                className={selectClass}
              >
            ...
            <label
              ...
              className="mt-4 flex items-start gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-4 text-sm text-slate-700"
            >
              <input
                id="deletion-confirm"
                ...
                onChange={(event) =>
                  setDeletionConfirmChecked(event.target.checked)
                }
                disabled={isGovernanceSubmitting}
            ...
            <div className="mt-4 space-y-3">
              <p className={infoRowClass}>
                ...
                {deletionRequest?.status ?? "not requested"}
              </p>
              <p className={infoRowClass}>
              ...
              {deletionRequest?.due_at ? (
                <p className={infoRowClass}>
                  Deletion due at: {deletionRequest.due_at}
                </p>
              ) : null}
              {deletionRequest?.completed_at ? (
                <p className={infoRowClass}>
                  Deletion completed at: {deletionRequest.completed_at}
                </p>
              ) : null}
              {deletionRequest?.error_code ? (
                <p className={infoRowClass}>
                  Deletion error code: {deletionRequest.error_code}
                </p>
              ) : null}
            </div>
          </section>
      ...
      {successMessage ? (
        <p
          role="status"
          className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-4 text-sm text-emerald-700"
        >
          {successMessage}
        </p>
      ) : null}
