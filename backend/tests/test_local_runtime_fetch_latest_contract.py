...
def test_fetch_latest_command_supports_latest_offset(monkeypatch, capsys, tmp_path: Path) -> None:
    db_path = tmp_path / "local-runtime-fetch-latest-offset.db"
    ...
    def _stub_fetch(url: str, timeout_seconds: float) -> bytes:
        ...
        if "/events?" in url or url.endswith("/events"):
            return (
                '{"value":['
                '{"id":3,"eventName":"City Council Meeting","eventDate":"2026-03-09T00:00:00Z",'
                '"publishedFiles":[{"type":"Minutes","name":"March 9 Minutes","fileId":1306,"url":"stream/mar-9-minutes.pdf"}]},'
                '{"id":2,"eventName":"City Council Meeting","eventDate":"2026-02-11T00:00:00Z",'
                '"publishedFiles":[{"type":"Minutes","name":"Approved Minutes","fileId":1305,"url":"stream/new-minutes.pdf"}]}'
                ']}'
            ).encode("utf-8")
        if "GetMeetingFile(" in url and "plainText=true" in url:
    ...
    assert payload["error_summary"] is None
    assert payload["warnings"] == []
    assert payload["stage_outcomes"][0]["metadata"]["meeting_date"] == "2026-02-11"
