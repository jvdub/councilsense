...
- Additive schema specification document: `docs/runbooks/st-022-additive-schema-migration-sequence-plan.md`.
- Ordered migration sequence plan with rollback-safe and additive-only ordering.
- Constraint/index matrix tied to ingestion dedupe and API lookup use cases.

## Completion Notes

- Authored canonical entity schema plan for documents, artifacts, spans, evidence linkage extensions, and publication/source-coverage aggregates.
- Migration sequencing is explicitly additive-only, includes backfill ordering, and uses flag-first rollback safety (disable writes/readers rather than destructive schema reversal).
- Destructive operations are explicitly prohibited in scope for this task.
